...
double bradleyterryeasy(int a, int b, int id, List lambda, List datset ){
  ...
  DataFrame datsetID=as<DataFrame>(datset[id-1]); //create dataframe for subsetting in loop; id-1= 4990 due to indexing at 0? 
  DataFrame lambdaID=as<DataFrame>(lambda[id-1]); //create dataframe for subsetting in loop 
  ...
  double output = (a-1+sum(datsetID[2])/(b+summationterm);
  return(output)
  }
